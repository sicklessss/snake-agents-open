...
class GameRoom {
    ...
    _ensurePredictionSlots() {
        ...
        // FIX #2: TARGET_VISIBLE_FUTURE reduced 5→3. Combined with on-chain
        // throttling below, this caps maintained-ahead matches per room at
        // ~3 instead of ~5, cutting createMatch enqueue rate by ~40% during
        // backlog conditions. UX impact: prediction panel shows next+1
        // matches confirmed on-chain instead of next+4 — manageable.
        ...
        // FIX #3: Hard back-pressure with hysteresis + escape valve.
        // - Pause new createMatch enqueues when queue > HIGH (500)
        // - Resume only after queue drains below LOW (200) [hysteresis]
        // - Escape: if pause persists > 5 min with NO successful createMatch
        //   in that window, force one through anyway to avoid silent stall
        //   (frontend would have nothing to predict on if we never push).
        const QUEUE_HIGH = 500, QUEUE_LOW = 200;
        if (queueLen > QUEUE_HIGH) this._queuePaused = true;
        if (queueLen < QUEUE_LOW) this._queuePaused = false;
        if (this._queuePaused) {
            const sinceLastSuccess = Date.now() - (this._lastSuccessfulCreateAt || 0);
            const ESCAPE_MS = 5 * 60_000;
            if (sinceLastSuccess > ESCAPE_MS) {
                C.log.warn(`[PredictionSlots] queue=${queueLen} but no successful createMatch in ${(sinceLastSuccess/1000).toFixed(0)}s — escape valve, allowing 1 push`);
                // fall through with onChainSlots=1
            } else {
                if (queueLen > 1000 && queueLen % 100 === 0) {
                    C.log.warn(`[PredictionSlots] queue=${queueLen} (paused, escape in ${((ESCAPE_MS-sinceLastSuccess)/1000).toFixed(0)}s)`);
                }
                return; // skip enqueue this cycle
            }
        }
        ...
        // FIX #2: Lower on-chain push targets so we don't pile work onto
        // an already-loaded queue. Combined with TARGET_VISIBLE_FUTURE=3,
        // a healthy room maintains 1–3 confirmed on-chain matches.
        //   queue <= 30  → push 3 slots
        //   queue <= 60  → push 2 slots
        //   queue >  60  → push 1 slot (or escape-valve mode)
        let onChainSlots;
        if (this._queuePaused) onChainSlots = 1; // escape valve
        else if (queueLen <= 30) onChainSlots = 3;
        else if (queueLen <= 60) onChainSlots = 2;
        else onChainSlots = 1;
        ...
        entries.forEach((entry, idx) => {
            ...
            // FIX #1: pass startTime as a closure that recomputes at TX
            // execution time. Without this, backlog → stale startTime →
            // contract revert → ethers retry with same stale value → 10
            // failures → CIRCUIT BREAKER (the original cascade source).
            const startTimeFn = () => this._estimateFutureSlotStartTime(idx + 1);
            this._ensureMatchCreated(entry, startTimeFn, `prediction-slot-${idx + 1}`);
        });
    }
