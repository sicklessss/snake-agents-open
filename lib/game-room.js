...
class GameRoom {
    ...
    _ensurePredictionSlots() {
        ...
        // HIGH-2 Plan A: pre-create more matches ahead of time so prediction intents
        // always find a confirmed on-chain match, even during TX queue backlogs.
        // nextMatch + 4 futureMatches = 5 visible slots ahead of the current match.
        const TARGET_VISIBLE_FUTURE = 5;
        ...
        // Threshold raised from 20 to 100: after the idempotency fix the steady
        // state sits at ~20–40 items (one entry per active prediction slot TX
        // in flight). 20 was triggering 80k false warnings/day. Keep the warn
        // as a real-backlog signal, not a nominal-traffic alarm.
        if (queueLen > 100) {
            C.log.warn(`[PredictionSlots] TX queue backlog: ${queueLen} items`);
        }
        // Pre-create up to 4 slots on-chain so the executor always has a confirmed match.
        // Graceful throttling as backlog grows:
        //   queue <= 30  → push all 4 slots
        //   queue <= 60  → push 3 slots
        //   queue <= 100 → push 2 slots
        //   queue  > 100 → push 1 slot (minimum to keep system running)
        let onChainSlots;
        if (queueLen <= 30) onChainSlots = 4;
        else if (queueLen <= 60) onChainSlots = 3;
        else if (queueLen <= 100) onChainSlots = 2;
        else onChainSlots = 1;
