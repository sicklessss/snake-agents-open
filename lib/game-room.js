...
// Seeded PRNG: mulberry32. Pure function of (seed, callIndex) — byte-identical
// across Node versions and verifier implementations. Must NOT change.
...
class GameRoom {
    constructor({ id, type, letter }) {
        ...
        // Seeded RNG — ensures physics is deterministic given matchId.
        // Public verifier (snake-agents-open) re-runs physics with the same seed
        // and same bot inputs to prove the replay matches the published engine.
        this._rngSeed = this.currentMatchId >>> 0;
        this.rng = mulberry32(this._rngSeed);
        // Per-tick bot input log: [{ tick, playerId, dir: {x, y} }, ...]
        // Populated by handleMove(), consumed by replay-recorder for verifiability.
        this.inputLog = [];
        // Out-of-band events that happen between ticks (disconnects, etc.).
        // The verifier applies these at the recorded turn to mirror the server.
        this.eventLog = [];
    ...
    startLoops() {
        ...
        this._secondInterval = setInterval(() => {
            ...
            if (this.gameState === 'PLAYING') {
                ...
                // Spawn performance workers 20s before match start. Older value
                // (5s) was too tight — isolate boot + WS handshake regularly
                // exceeded 5s under event-loop lag, leaving bots as ws=null
                // floodFillMove placeholders that all starve simultaneously
                // → No-Winner outcome.
                if (this.gameState === 'COUNTDOWN' && !this._countdownSpawnTriggered && this.timerSeconds <= 20) {
                ...
                if (this.gameState === 'GAMEOVER') {
                    ...
                    // STARTING phase (wait-for-ready) was rolled back here:
                    // in A/B testing with perf-A, every attempt reported
                    // 10/10 not ready — suggests the ws:null entries seen
                    // during STARTING are from backfill, NOT the handshake
                    // status of already-spawned workers. Need to debug the
                    // data source before re-enabling. UI indicator itself
                    // (preparing/ready in Fighters list) stays since it
                    // correctly reflects the server's view.
                    this.startGame();
                } else {
                    this.timerSeconds = 0;
                }
    ...
    _ensureMatchCreated(entry, startTimeOrFn, reason = 'scheduled') {
        ...
        // CASCADE FIX: startTime can be either a number (legacy) or a closure
        // that recomputes startTime at TX-execution time. Closure form is
        // strongly preferred — when the queue is backed up by minutes, the
        // closure version stays fresh while a captured number goes stale and
        // triggers contract revert "Start time must be in future" → ethers
        // retries with same stale value → 10 fails → CIRCUIT BREAKER →
        // queue grows further. The closure form + safety clamp eliminates
        // this entire cascade.
        const room = this;
        BC.enqueueTxPriority(`createMatch ${targetMatchId} (${reason})`, async (overrides) => {
            try {
                ...
                if (existingId !== 0) {
                    ...
                    room._autoBindDisplayMatch(displayMatchId, targetMatchId);
                    return;
                }
            } catch (_) {}
            try {
                // Re-evaluate startTime at TX execution time. Clamp to at least
                // now+60 so the contract's `_startTime > block.timestamp` check
                // always passes even if the closure returns a past value.
                let startTime;
                try {
                    startTime = typeof startTimeOrFn === 'function'
                        ? Number(startTimeOrFn())
                        : Number(startTimeOrFn);
                } catch (_) { startTime = 0; }
                const minStart = Math.floor(Date.now() / 1000) + 60;
                if (!Number.isFinite(startTime) || startTime < minStart) startTime = minStart;
                ...
                tx.wait(1, 60000).then(() => {
                    entry.chainCreated = true;
                    entry.chainCreateQueued = false;
                    // Track last CONFIRMED createMatch (not just sent) for the
                    // queue-pause escape valve. If sends keep happening but
                    // none confirm (cascade returning, mempool drops, etc.),
                    // this stays stale → escape valve fires sooner, surfacing
                    // the regression instead of silently retrying forever.
                    room._lastSuccessfulCreateAt = Date.now();
    ...
    _estimateFutureSlotStartTime(slotIndex) {
        ...
        if (this.gameState === 'COUNTDOWN' || this.gameState === 'STARTING') {
            // STARTING behaves like a countdown of up to 15 s while we wait
            // for straggler workers to WS-handshake.
    ...
    _ensurePredictionSlots() {
        ...
        // FIX #2 (rev 3): restored to 3 after Phase 2 (lazy on-chain createMatch).
        // Now that future-match reservations no longer trigger createMatch txs
        // (those happen lazily on first prediction intent), keeping more visible
        // slots is FREE — they're pure in-memory bookkeeping. UX win: prediction
        // panel shows current + next + 2 future quick-buttons per arena.
        const TARGET_VISIBLE_FUTURE = 3;
        ...
        // PHASE 2: lazy on-chain createMatch.
        // We no longer preemptively createMatch for slot reservations. A match
        // only goes on-chain when the first prediction intent arrives for it
        // (triggered from routes/intent-api.js via promoteMatchOnChain()).
        // This eliminates ~95% of wasted createMatch txs on empty matches.
        // Slot reservation in memory (above) is still needed so the prediction
        // panel can display the next matchId for users to bet on.
    }

    // PHASE 2: external entry point used by intent submission API to lazy-promote
    // a reserved (in-memory only) match slot to on-chain when the first
    // prediction intent arrives. Idempotent — safe to call multiple times.
    // Handles three locations the entry can live in:
    //   1. _currentMatchEntry: the match in COUNTDOWN/PLAYING/GAMEOVER
    //   2. nextMatch:          the match after the current one
    //   3. futureMatches[i]:   matches further ahead (rare with TARGET_VISIBLE_FUTURE=1)
    promoteMatchOnChain(matchId, reason = 'intent-trigger') {
        if (!S.pariMutuelContract) return false;
        let entry = null;
        let slotIdx = -1; // 0=current, 1=next, 2+=future
        if (this._currentMatchEntry && this._currentMatchEntry.matchId === matchId) {
            entry = this._currentMatchEntry; slotIdx = 0;
        } else if (this.nextMatch && this.nextMatch.matchId === matchId) {
            entry = this.nextMatch; slotIdx = 1;
        } else {
            for (let i = 0; i < this.futureMatches.length; i++) {
                if (this.futureMatches[i].matchId === matchId) {
                    entry = this.futureMatches[i]; slotIdx = i + 2; break;
                }
            }
        }
        if (!entry) return false;
        if (entry.chainCreated || entry.chainCreateQueued) return true;
        const startTimeFn = () => this._estimateFutureSlotStartTime(slotIdx + 1);
        this._ensureMatchCreated(entry, startTimeFn, reason);
        return true;
    }
    ...
    tick() {
        // Idempotency: tick() can be called by catch-up loop even after
        // gameState has just transitioned to GAMEOVER in the prior iteration
        // of the same while(). Without this guard, wipeout triggered 4x in a
        // single catch-up window, each time calling startGameOver → duplicate
        // saveReplay / history / settleMatch. Observed as 3 extra
        // "Empty frames" WARN per finished match and spurious cancelMatch TXs.
    ...
    startGameOver(survivor, reason = 'last_survivor') {
        // Idempotency guard — defense in depth behind tick()'s check. Any caller
        // (endMatchByTime, wipeout in processTick, victoryPauseTimer) must only
        // finalize once per match. Without this, duplicate settleMatch TXs fire,
        // spurious history rows are saved, and replayFrames is re-saved empty.
        ...
        const participants = Object.values(this.players)
            ...
            .map(p => p.name);
        // Pass currentMatchId so history.matchId matches the replay filename
        // and /api/replay/:id lookups from history rows resolve correctly.
        P.saveHistory(this.id, this.winner, survivor ? survivor.score : 0, null, participants, this.currentMatchId);
        ...
        // PHASE 2: removed preemptive createMatch for nextMatch.
        // Was: each match's createMatch was triggered ~60s before its startTime
        // (immediately when previous match's GAMEOVER fired). Now lazy — the
        // match only goes on-chain if a prediction intent arrives (handled by
        // intent-api.js → room.promoteMatchOnChain()). Empty matches stay
        // entirely off-chain, saving createMatch + lockBetting + settleMatch.

        // PHASE 3: skip the entire settle/cancel path if match was never put
        // on-chain (lazy createMatch never triggered = no predictions arrived).
        // Without this guard, the settle queue keeps retrying matches() RPC for
        // 30 min then gives up — flooding logs and tripping the CIRCUIT BREAKER
        // when RPC 502 errors mask the real "match doesn't exist" reason.
        const _wasOnChain = S._matchChainStatus[this.currentMatchId]?.create === 'confirmed';
        if (S.pariMutuelContract && !_wasOnChain) {
            C.log.info(`[Blockchain] match #${this.currentMatchId}: lazy/never-created — skipping settle entirely`);
        }
        ...
        if (S.pariMutuelContract && _wasOnChain) {
            ...
            if (settlementWinnerBotIds.length > 0) {
                ...
                let settleDefers = 0;
                let settleRetries = 0;
                const MAX_SETTLE_RETRIES = 3; // max on-chain TX retries after revert
                const _waitAndSettle = async () => {
                    try {
                        ...
                        // Filter: already settled or cancelled → don't waste gas
                        const settled = matchData.settled || matchData[5] || false;
                        const cancelled = matchData.cancelled || matchData[6] || false;
                        if (settled || cancelled) {
                            C.log.debug(`[Blockchain] settleMatch #${onChainMatchId}: already ${settled ? 'settled' : 'cancelled'}, skipping`);
                            S.pendingSettlements = S.pendingSettlements.filter(s => s.matchId !== onChainMatchId);
                            P.savePendingSettlements();
                            return;
                        }
                        // Filter: no bets → cancel instead of settle (saves gas, avoids revert)
                        const totalPool = BigInt(matchData.totalPool || matchData[3] || 0);
                        if (totalPool === 0n) {
                            C.log.debug(`[Blockchain] settleMatch #${onChainMatchId}: no bets in pool, skipping settle`);
                            S.pendingSettlements = S.pendingSettlements.filter(s => s.matchId !== onChainMatchId);
                            P.savePendingSettlements();
                            return;
                        }
                    ...
                    // Match is on-chain, not yet settled, has bets, startTime passed — enqueue actual TX
                        ...
                        tx.wait(1, 120000).then(async (receipt) => {
                            if (receipt.status !== 1) {
                                ...
                                if (settleRetries >= MAX_SETTLE_RETRIES) {
                                    C.log.error(`[Blockchain] settleMatch #${onChainMatchId} TX reverted ${settleRetries} times, giving up`);
                                    S.pendingSettlements = S.pendingSettlements.filter(s => s.matchId !== onChainMatchId);
                                    P.savePendingSettlements();
                                } else {
                                    C.log.error(`[Blockchain] settleMatch #${onChainMatchId} TX reverted (attempt ${settleRetries}/${MAX_SETTLE_RETRIES})`);
                                }
                                return;
                            }
                            ...
                            try {
                                ...
                                if (!settled) {
                                    settleRetries++;
                                    if (settleRetries >= MAX_SETTLE_RETRIES) {
                                        C.log.error(`[Blockchain] settleMatch #${onChainMatchId} receipt OK but not settled after ${settleRetries} retries, giving up`);
                                        S.pendingSettlements = S.pendingSettlements.filter(s => s.matchId !== onChainMatchId);
                                        P.savePendingSettlements();
                                        return;
                                    }
                                    C.log.error(`[Blockchain] settleMatch #${onChainMatchId} receipt OK but not settled — retry ${settleRetries}/${MAX_SETTLE_RETRIES}`);
                                    _waitAndSettle();
                                    return;
                                }
                            } catch (_) {}
                            ...
                            settleRetries++;
                            if (settleRetries >= MAX_SETTLE_RETRIES) {
                                C.log.error(`[Blockchain] settleMatch #${onChainMatchId} TX wait failed ${settleRetries} times: ${e.message} — giving up`);
                                S.pendingSettlements = S.pendingSettlements.filter(s => s.matchId !== onChainMatchId);
                                P.savePendingSettlements();
                            } else {
                                C.log.error(`[Blockchain] settleMatch #${onChainMatchId} TX wait failed (attempt ${settleRetries}/${MAX_SETTLE_RETRIES}): ${e.message}`);
                                _waitAndSettle();
                            }
                        });
                    });
    ...
    startCountdown() {
        ...
        if (this.type === 'competitive') {
            ...
            // Also reset per-bot join-retry counters so each match gets fresh retries.
            const BM = require('./bot-manager');
            if (BM.resetCompetitiveJoinRetries) BM.resetCompetitiveJoinRetries();
        }
        ...
        // Spawning is staggered (200 ms apart globally via bot-manager's enqueueSpawn)
        // so 10 simultaneous `new Worker(...)` forks don't freeze the main thread for
        // ~2 s — that freeze used to push the slow-compiling bots (Alpha / India /
        // Foxtrot / Delta) past the 30 s join window.
        if (this.type === 'performance') {
            const BM = require('./bot-manager');
            const arenaId = this.id;
            for (const [id, w] of Object.entries(this.waitingRoom)) {
                if (w.botType === 'agent' && w.botId) {
                    ...
                    const botId = w.botId; // closure-capture
                    BM.enqueueSpawn(() => {
                        // Skip if a worker is already running OR is mid-shutdown
                        // (the 2 s force-terminate window). Skip too if a
                        // competitive worker was just started for this bot —
                        // cross-arena cooldown prevents double-boot contention.
                        if (!S.activeWorkers.has(botId) &&
                            !S.stoppingWorkers.has(botId) &&
                            !BM.isSpawnCoolingDown(botId)) {
                            BM.startBotWorker(botId, arenaId);
                        }
                    });
                }
            }
        ...
        this.players = {};
        // Consume pre-created nextMatch if available. PHASE 2: we keep the
        // entry on this._currentMatchEntry so that intent-driven lazy promotion
        // can still find it after consumption (intents may arrive during the
        // COUNTDOWN that follows this transition).
        if (this.nextMatch) {
            this.currentMatchId = this.nextMatch.matchId;
            this.displayMatchId = this.nextMatch.displayMatchId;
            this._currentMatchEntry = this.nextMatch;
            C.log.important(`[Room ${this.id}] Consumed nextMatch #${this.currentMatchId} (${this.displayMatchId}), chainCreated=${this.nextMatch.chainCreated} (lazy)`);
            // PHASE 2: removed the always-on retry. With lazy createMatch,
            // chainCreated=false simply means "no intent has triggered it yet"
            // — not a failure condition. promoteMatchOnChain() handles the
            // on-demand path when an intent arrives.
        } else {
            // Fallback: nextMatch slot wasn't reserved. Allocate matchId in
            // memory only — still NO automatic createMatch (lazy). If an
            // intent arrives, the intent submit handler will trigger it.
            this.currentMatchId = P.nextMatchId();
            this.displayMatchId = P.getNextDisplayId(this.type, this.letter);
            P.registerDisplayId(this.displayMatchId, this.currentMatchId);
            this._currentMatchEntry = {
                matchId: this.currentMatchId,
                displayMatchId: this.displayMatchId,
                chainCreated: false,
                chainCreateQueued: false,
            };
            C.log.important(`[Room ${this.id}] Allocated fallback match #${this.currentMatchId} (${this.displayMatchId}) in memory only (lazy)`);
        }
    ...
    startGame() {
        ...
        // Per-match worker audit: count agents/normals and flag any agent that
        // entered without a live worker (silent floodfill fallback). One grep
        // for `[MatchAudit ` shows whether every agent slot is truly worker-driven.
        try {
            const auditAgentsWorker = [];
            const auditAgentsFloodfill = [];
            const auditNormals = [];
            for (const w of Object.values(this.waitingRoom)) {
                if (w.botType === 'normal') {
                    auditNormals.push(w.name);
                } else if (w.botType === 'agent' || w.botType === 'hero') {
                    // Count both OPEN (1) and CONNECTING (0) as worker-present.
                    // A worker that just spawned can be in CONNECTING for a few
                    // ms before the WS handshake completes — without this we'd
                    // print spurious "silent fallback" warns for races between
                    // startGame and the new socket flipping to OPEN.
                    if (w.ws && (w.ws.readyState === 0 || w.ws.readyState === 1)) {
                        auditAgentsWorker.push(`${w.name}(${w.botId || w.id})`);
                    } else {
                        auditAgentsFloodfill.push(`${w.name}(${w.botId || w.id})`);
                    }
                }
            }
            const matchTag = `#${this.currentMatchId}/${this.displayMatchId || '-'}`;
            const summary = `agents=${auditAgentsWorker.length + auditAgentsFloodfill.length}` +
                ` (worker=${auditAgentsWorker.length}, floodfill-fallback=${auditAgentsFloodfill.length})` +
                `, normals=${auditNormals.length}`;
            if (auditAgentsFloodfill.length > 0) {
                C.log.warn(`[MatchAudit ${matchTag} ${this.id}] ${summary} → silent fallback: [${auditAgentsFloodfill.join(', ')}]`);
            } else {
                C.log.important(`[MatchAudit ${matchTag} ${this.id}] ${summary}`);
            }
        } catch (e) {
            C.log.warn(`[MatchAudit] failed to emit audit log: ${e.message}`);
        }
        ...
        this.timerSeconds = 0;
        this.matchTimeLeft = C.MATCH_DURATION;
        // Re-seed RNG with the actual matchId for THIS match. Ensures verifier
        // reproduces physics deterministically even after prediction-slot rotation.
        this._rngSeed = this.currentMatchId >>> 0;
        this.rng = mulberry32(this._rngSeed);
        this.inputLog = [];
        this.eventLog = [];
        // Snapshot pre-tick state for verifiable replay. Spawn assignment uses
        // unseeded Math.random() so the verifier cannot reconstruct it from the
        // seed alone — capture the post-spawn / pre-tick-1 state here.
        // hasController distinguishes ws-driven bots (inputs come from inputLog)
        // from ws=null bots (verifier must run floodFillMove to mirror server).
        this._initialState = {
            ...
            // Snapshot actual obstacles present at match start. In competitive
            // mode startCountdown clears them, so this is usually [] — but
            // serializing the real array rather than a literal keeps the
            // replay metadata structurally correct for any future variant
            // that pre-seeds obstacles.
            obstacles: (this.obstacles || []).map(o => ({
                x: o.x, y: o.y, solid: !!o.solid, blinkTimer: o.blinkTimer | 0,
            })),
        };
        this.phaseEndsAt = null;
        this.matchEndsAt = Date.now() + (C.MATCH_DURATION * 1000);
        this._countdownSpawnTriggered = false;
        // PHASE 3b: lockBetting tx removed entirely. The new PariMutuel contract
        // (deployed 2026-04-27) no longer reads `bettingLocked` in placeBet/placeBetFor;
        // it relies on `block.timestamp < startTime + BETTING_WINDOW (=0)` for
        // closure, so an explicit lock-tx is redundant. Saves 1 tx per match
        // that had any predictions. The contract's lockBetting() is still
        // present (no-op flag setter) for backwards compat but never called.
    ...
    broadcastState() {
        ...
        const waitingPlayers = Object.values(this.waitingRoom).map((w) => ({
            ...
            alive: true,
            waiting: true,
            // ready=true once the bot's worker has handshook on WS. Frontend
            // shows "preparing…" → "ready" so viewers can see prep progress.
            // Normal/floodfill bots have no worker (server drives them via
            // floodFillMove) — they're always ready as soon as seeded.
            ready: w.botType === 'normal' ? true : !!w.ws,
            botType: w.botType,
            botId: w.botId || w.id,
        }));

        // Betting is open only before the match starts.
        // Betting stays open through STARTING (the "waiting for last bots"
        // sub-phase) since the match hasn't physically started yet.
        const bettingOpen = !S.bettingDisabled && (this.gameState === 'COUNTDOWN' || this.gameState === 'STARTING');
    ...
    handleMove(playerId, data) {
        if (playerId && this.players[playerId] && this.players[playerId].alive) {
            ...
            if (!BM.isOpposite(newDir, p.direction)) {
                ...
                // Record input for verifiable replay. Only log when gameState is
                // PLAYING — pre-match inputs have no physics effect.
                if (this.gameState === 'PLAYING' && this.inputLog && this.inputLog.length < 50000) {
                    this.inputLog.push({ t: this.turn, p: playerId, x: newDir.x, y: newDir.y });
                }
            }
        }
    }
    ...
    handleDisconnect(playerId) {
        if (playerId) {
            ...
            if (this.players[playerId]) {
                // Log disconnect for verifiable replay — happens out-of-band,
                // so the verifier needs to mirror it before the next tick runs.
                if (this.gameState === 'PLAYING' && this.eventLog && this.eventLog.length < 10000) {
                    this.eventLog.push({ t: this.turn, type: 'disconnect', p: playerId });
                }
                this.killPlayer(this.players[playerId], 'disconnect');
            }
