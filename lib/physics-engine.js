...
function processTick(room) {
    ...
    while (room.food.length < foodCap) {
        ...
        let fx, fy;
        do {
            fx = Math.floor(Math.random() * C.CONFIG.gridSize);
            fy = Math.floor(Math.random() * C.CONFIG.gridSize);
            tries++;
            if (tries > 200) break;
...
function spawnObstacle(room) {
    ...
    do {
        seedX = Math.floor(Math.random() * (C.CONFIG.gridSize - 4)) + 2;
        seedY = Math.floor(Math.random() * (C.CONFIG.gridSize - 4)) + 2;
        tries++;
